...
{	
	...
	then
		...
		echo "$("nipe is not installed")" | lolcat -af "-> [*] Installing nipe" | lolcat -af
		# Downloading
		...
		echo "$(figlet "nipe is installed")" | lolacat -af
		echo "-----------------------------------------------------" | lolcat -af
		echo " :):):):):):):):):):):):):):):):):):):):):):):):):):):):):):) " | lolcat -af
	...
	then
		echo "[*] sshpass is installed" | lolcat -af
		echo "-----------------------------------------------------" | lolcat -af
	else
...
if [ "$(whoami)" == "root" ]
	then
		...
		sudo apt -y install kali-root-login 1>/dev/null
		sudo su
		kali
